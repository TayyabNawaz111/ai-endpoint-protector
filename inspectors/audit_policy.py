...
def inspect() -> dict:
    ...
    if returncode != 0:
        # Check if it's a privilege issue
        if "privilege" in stderr.lower() or "0x00000522" in stderr:
            return {
                "parameter": "audit_policy",
                "logon_auditing": None,
                "success_audited": False,
                "failure_audited": False,
                "raw_output": stdout,
                "error": "Administrator privileges required to check audit policy (auditpol requires elevation).",
            }
        ...
        return {
            ...
            "failure_audited": False,
            "raw_output": stdout,
            "error": f"auditpol command failed: {stderr.strip() if stderr else 'Unknown error'}",
        }
